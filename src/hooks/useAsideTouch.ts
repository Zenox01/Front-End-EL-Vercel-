...
const useAsideTouch = () => {
  ...
  const hasTouchButton =
    typeof deviceScreen?.width !== "undefined" &&
    deviceScreen?.width >
      Number(process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE);
  ...
  const x = useMotionValue(
    process.env.REACT_APP_ASIDE_TOUCH_STATUS === "true"
      ? 0
      : asideWidthWithSpace
  );
  ...
  const asideStyle = hasTouchButton
    ? { left }
    : { left: mobileDesign ? undefined : "0rem" };
